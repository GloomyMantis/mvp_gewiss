...
import { Upload, X, FileText, CheckCircle2, ArrowLeft, Plus, Trash2 } from 'lucide-react'
...
export default function NewProjectPage() {
  ...
  const removeFile = (idx: number) => {
    setBoqFiles(prev => prev.filter((_, i) => i !== idx))
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const { file, displayName } of boqFiles) {
        ...
        const fileName = `${user.id}/${project.id}/${Date.now()}.${ext}`
  ...
  return (
    <div className="p-8 max-w-xl mx-auto">
      <div className="mb-8">
        <Link href="/dashboard" className="inline-flex items-center gap-1.5 text-sm text-gray-400 hover:text-gray-600 mb-4 transition-colors">
